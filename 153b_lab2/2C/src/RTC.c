...
void RTC_Init(void) {
	...
	// weekday (1 = Monday), day, month, year
	RTC_Set_Calendar_Date(3, 23, 4, 2025);
	
	// Configure the Time
	// format (0 = 24 hour/PM), hour, minute, second
	RTC_Set_Time(0, 16, 10, 30);
...
void RTC_Set_Calendar_Date(uint32_t WeekDay, uint32_t Day, uint32_t Month, uint32_t Year) {
	RTC->DR = (getTens(Year) << 20 | getOnes(Year) << 16 | WeekDay << 13 | getTens(Month) << 12 | getOnes(Month) << 8 | getTens(Day) << 4 | getOnes(Day));
}

void RTC_Set_Time(uint32_t Format12_24, uint32_t Hour, uint32_t Minute, uint32_t Second) {
	RTC->TR = (Format12_24 << 22 | getTens(Hour) << 20 | getOnes(Hour) << 16 | getTens(Minute) << 12 | getOnes(Minute) << 8 | getTens(Second) << 4 | getOnes(Second));
}

uint32_t RTC_TIME_GetHour(void) {
	return (RTC->TR & (RTC_TR_HT | RTC_TR_HU)) >> 16;
}

uint32_t RTC_TIME_GetMinute(void) {
	return (RTC->TR & (RTC_TR_MNT | RTC_TR_MNU)) >> 8;
}
...
uint32_t RTC_DATE_GetMonth(void) {
	return (RTC->DR & (RTC_DR_MT | RTC_DR_MU)) >> 8;
}

uint32_t RTC_DATE_GetDay(void) {
	return RTC->DR & (RTC_DR_DT | RTC_DR_DU);
}

uint32_t RTC_DATE_GetYear(void) {
	return (RTC->DR & (RTC_DR_YT | RTC_DR_YU)) >> 16;
}

uint32_t RTC_DATE_GetWeekDay(void) {
	return (RTC->DR & (RTC_DR_WDU)) >> 12;
}
